...
export default function AdminUsersPage() {
  ...
  return (
    <AdminShell
      ...
      headerActions={
        <a className="btn" href="/register" style={{ fontSize: "0.85rem", padding: "0.5rem 1rem" }}>
    ...
    >
      <section className="neo-card" style={{ overflow: "hidden" }}>
        <div className="row" style={{ padding: "1.25rem 1.5rem", flexWrap: "wrap", gap: "0.75rem" }}>
        ...
        <div className="table-shell" style={{ border: "none", borderRadius: 0, boxShadow: "none" }}>
          <table>
            ...
            <tbody>
              {displayed.map((user) => {
                const rc = roleConfig[user.role] ?? { cls: "pill", label: user.role };
                return (
                  <tr key={user.email}>
                    <td>
                      <div className="row">
                    ...
                    <td>
                      <button type="button" className="btn-ghost" style={{ padding: "0.35rem 0.75rem", fontSize: "0.8rem" }}>
                        Detail
                      </button>
                    </td>
                  </tr>
              ...
              {loading && (
                <tr><td colSpan={5}>Memuat data pengguna...</td></tr>
              )}
              {error && (
                <tr><td colSpan={5} style={{ color: "var(--rose)" }}>{error}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {users.length > 0 && (
          <div className="row" style={{ justifyContent: "flex-end", padding: "1rem 1.5rem", gap: "0.5rem" }}>
            <span className="text-dim" style={{ fontSize: "0.8rem", marginRight: "auto" }}>
              Menampilkan {displayed.length} dari {users.length} Pengguna
            </span>
            <button className="btn-ghost" style={{ padding: "0.4rem 1rem", fontSize: "0.8rem" }}>Prev</button>
            <button className="btn" style={{ padding: "0.4rem 1rem", fontSize: "0.8rem" }}>Next</button>
          </div>
        )}
      </section>
    </AdminShell>
