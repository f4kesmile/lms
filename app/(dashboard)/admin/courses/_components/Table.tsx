        ...
        <UITable>
          ...
          <TableBody>
            {loading ? (
              Array(5)
                ...
                .map((_, index) => (
                  <TableRow key={`skeleton-${index}`}>
                    <TableCell colSpan={activeTab === "years" ? 4 : 5} className="h-20">
              ...
              <TableRow>
                <TableCell colSpan={activeTab === "years" ? 4 : 5} className="p-12">
                  <EmptyState
                    icon={activeTab === "mataKuliah" ? GraduationCap : activeTab === "kelas" ? LayoutGrid : Calendar}
                    title={activeTab === "mataKuliah" ? "Belum ada mata kuliah" : activeTab === "kelas" ? "Belum ada kelas" : "Belum ada tahun akademik"}
                    description={
                      searchQuery
                        ...
                        <>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              className={cn(
                                ...
                                subject.status === "published" ? "bg-emerald-500 hover:bg-emerald-600" : ""
                              )}
                            >
                              {subject.status === "published" ? "AKTIF" : subject.status.toUpperCase()}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            <span className="text-[11px] text-muted-foreground font-black uppercase tracking-widest opacity-60">
                               {new Date(subject.updatedAt).toLocaleDateString("id-ID", { day: '2-digit', month: 'short', year: 'numeric' })}
                            </span>
                          </TableCell>
                        </>
                      );
                        ...
                        <>
                          <TableCell className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              ...
                              <p className="text-sm font-black tracking-tight">{classItem.name}</p>
                            </div>
                          </TableCell>
                          <TableCell>
                             <Badge variant="secondary" className="bg-primary/5 text-primary border-primary/20 font-black text-[10px] uppercase tracking-wider">
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                               ...
                               <span className="text-sm font-bold text-muted-foreground">
                                 {classItem.classTeacher?.name || "Belum ada dosen"}
                               </span>
                            </div>
                          </TableCell>
                        ...
                        <>
                          <TableCell className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              ...
                              <p className="text-sm font-black tracking-tight">{year.name}</p>
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-center">
                            {year.isCurrent ? (
                              <div className="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 shadow-sm">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest leading-none">TAHUN AKTIF</span>
                              </div>
                            ) : (
                              <Button 
                                variant="outline" 
