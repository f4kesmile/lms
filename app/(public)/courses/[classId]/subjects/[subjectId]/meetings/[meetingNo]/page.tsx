...
export default async function MeetingContentPage({
  ...
  const nextMeeting = meetingNo < meeting.subject._count.meetings
    ? await prisma.subjectMeeting.findFirst({
        where: { subjectId, meetingNo: meetingNo + 1 },
        select: { id: true, meetingNo: true, title: true }
      })
    ...
    <>
      ...
      <main className="app-shell flex flex-col gap-8 pb-32">
        <nav className="flex items-center gap-2 text-[0.85rem] text-muted-foreground">
          <Link href={`/courses/${classId}` as Route} className="hover:text-primary transition-colors">
            Beranda Kelas
          </Link>
          <Icon name="chevron_right" size={16} />
          <Link href={`/courses/${classId}/subjects/${subjectId}` as Route} className="hover:text-primary transition-colors">
            {meeting.subject.name}
          </Link>
          <Icon name="chevron_right" size={16} />
          <span className="text-foreground font-semibold">Sesi {meeting.meetingNo}</span>
        </nav>
           ...
           <article className="space-y-8">
              <header className="space-y-4">
                 <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-muted-foreground">{meeting.subject.code}</span>
                 </div>
                 <h1 className="text-4xl md:text-5xl font-black tracking-tight text-foreground">{meeting.title}</h1>
              </header>
              ...
              <div className="neo-card p-8 md:p-12 bg-card min-h-[400px]">
                 <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: renderMaterialHtml(meeting.content) }}
                 />
              </div>
              ...
              <footer className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-8">
                 {prevMeeting ? (
                    <Link 
                       href={`/courses/${classId}/subjects/${subjectId}/meetings/${prevMeeting.meetingNo}` as Route}
                    ...
                    >
                       <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-primary">Sesi Sebelumnya</span>
                       <span className="font-bold text-sm line-clamp-1">{prevMeeting.title}</span>
                    </Link>
                 ) : <div />}

                 {nextMeeting ? (
                    <Link 
                       href={`/courses/${classId}/subjects/${subjectId}/meetings/${nextMeeting.meetingNo}` as Route}
                    ...
                    >
                       <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-primary">Sesi Berikutnya</span>
                       <span className="font-bold text-sm line-clamp-1">{nextMeeting.title}</span>
                    </Link>
                 ) : <div />}
              </footer>
           </article>
           ...
           <aside className="space-y-6 lg:sticky lg:top-24">
              <Card className="p-6 rounded-[2rem] bg-gradient-to-br from-primary/5 to-transparent border-primary/20">
                 <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-black tracking-tight text-sm">Informasi Sesi</h3>
                 </div>
                 <div className="space-y-4">
                    <div className="flex justify-between items-center text-xs">
                       <span className="text-muted-foreground font-medium">Mata Kuliah</span>
                       <span className="font-black text-primary">{meeting.subject.code}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                       <span className="text-muted-foreground font-medium">Progress Sesi</span>
                       <span className="font-black">{meeting.meetingNo} / {meeting.subject._count.meetings}</span>
                    </div>
                 </div>
              </Card>
              ...
              <Card className="p-8 rounded-[2.5rem] bg-secondary-brand text-secondary-brand-foreground shadow-2xl shadow-secondary-brand/20">
                 ...
                 <h3 className="text-xl font-black tracking-tight mb-2">Butuh Bantuan?</h3>
                 <p className="text-xs font-medium leading-relaxed mb-6 opacity-90">
                    Gunakan tombol chat di pojok kanan bawah untuk bertanya seputar materi sesi {meeting.meetingNo} ini. AI kami telah mempelajari seluruh modul mata kuliah ini.
                 </p>
