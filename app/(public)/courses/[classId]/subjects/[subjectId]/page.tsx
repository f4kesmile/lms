        ...
        <header className="neo-card overflow-hidden">
          <div className="relative h-48 md:h-64 flex items-end p-8 overflow-hidden">
            ...
            <div className="relative z-10 space-y-2">
              <span className="px-2 py-1 rounded bg-secondary-brand text-secondary-brand-foreground text-[10px] font-black uppercase tracking-widest">
                {subject.code}
              </span>
              ...
              <p className="text-white/70 text-sm font-medium">
                Diampu oleh:{" "}
                {subject.teachers.map((t) => t.user.name).join(", ")}
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="grid-2 lg:grid-cols-[2fr_1fr] items-start">
          <section className="space-y-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-muted-foreground font-medium">
                Pilih sesi untuk mulai membaca materi dan berdiskusi dengan AI.
              </p>
            </div>
              ...
              {subject.meetings.length > 0 ? (
                subject.meetings.map((meeting) => (
                  <Link
                    key={meeting.id}
                    href={
                      `/courses/${classId}/subjects/${subjectId}/meetings/${meeting.meetingNo}` as Route
                    }
                  ...
                  >
                    <div className="flex items-center gap-5">
                      <div className="size-12 shrink-0 rounded-xl bg-muted flex flex-col items-center justify-center group-hover:bg-secondary-brand/10 transition-colors">
                        <span className="text-[10px] font-black leading-none opacity-50">
                          SESI
                        </span>
                      ...
                      <div className="flex flex-col">
                        <h4 className="font-black text-foreground group-hover:text-secondary-brand transition-colors line-clamp-1">
                          {meeting.title}
                        </h4>
                        <p className="text-xs text-muted-foreground">
                          Materi bacaan & asisten AI aktif
                        </p>
                      </div>
                    </div>
                    <Icon
                      name="arrow_forward"
                      size={20}
                      className="text-muted-foreground group-hover:text-secondary-brand transition-all group-hover:translate-x-1"
                    />
                  </Link>
                ))
              ) : (
                <div className="py-20 text-center bg-muted/20 rounded-3xl border-2 border-dashed border-border/50">
                  ...
                  <p className="font-bold text-muted-foreground text-sm">
                    Belum ada sesi pertemuan yang dijadwalkan.
                  </p>
                </div>
              )}
            </div>
          ...
          <aside className="space-y-6">
            <Card className="p-6 rounded-[2rem] border-2 border-secondary-brand/20 bg-secondary-brand/5">
              <h3 className="text-sm font-black uppercase tracking-widest text-secondary-brand mb-4">
            ...
            <Card className="p-6 rounded-[2rem] border-none bg-primary/10 shadow-xl shadow-primary/5">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed font-medium">
                Setiap materi sesi di mata kuliah ini telah diindeks ke dalam
                sistem AI. Kamu bisa bertanya apa saja seputar topik perkuliahan
                ini langsung dari halaman materi.
              </p>
            </Card>
          </aside>
        </div>
