        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium text-muted-foreground max-w-md">
              Pantau runutan jadwal mengajar Anda di setiap sesi pertemuan
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              {weekLabel && (
                <Badge
                  variant="outline"
                  className="font-black text-[10px] uppercase h-6 px-3 border-primary/50 text-primary"
                >
                  Minggu: {weekLabel}
                </Badge>
              )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: "Mata Kuliah Aktif", val: summary.totalSubjects },
              { label: "Kelas Diampu", val: summary.totalClasses },
              { label: "Total Mahasiswa", val: summary.totalStudents },
              {
                label: "Materi Terbaru Minggu Ini",
                val: summary.totalUpdatesThisWeek,
              },
            ].map((s, i) => (
              <div
                key={i}
                className="bg-card/40 border border-border/50 rounded-2xl p-3 min-w-[100px]"
              >
            ...
            {groupedByDay[selectedDay].length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 bg-card/20 border border-dashed border-border/50 rounded-[3rem]">
                ...
                <p className="text-sm text-center text-muted-foreground/50 max-w-xs mt-1 leading-relaxed">
                  Semua mata kuliah sudah terpetakan atau hari ini memang sedang
                  kosong.
                </p>
              </div>
            ) : (
